...
let resourceEditId=null,scheduleEditId=null,linkEditKey=null,pendingConfirm=null;
...
function renderReview(){
  ...
  $('rulesTable').innerHTML=table(['Estado','Inicio cada','Anticipación','Horizonte','Acciones'],rulesRows,'Todavía no guardaste reglas de reserva.');
  ...
  $('resourcesTable').innerHTML=table(['Nombre','Tipo','Estado','Acciones'],resourceRows,'Aún no hay personas o recursos activos.');
  ...
  $('schedulesTable').innerHTML=table(['Quién atiende','Día','Desde','Hasta','Acciones'],scheduleRows,'Aún no hay horarios registrados.');
  ...
  $('linksTable').innerHTML=table(['Servicio','Quién atiende','Estado','Acciones'],linkRows,'Todavía no hay servicios asignados.');

  document.querySelectorAll('.js-edit').forEach(b=>b.addEventListener('click',handleEdit));
  document.querySelectorAll('.js-delete').forEach(b=>b.addEventListener('click',handleDelete));
}
...
$('assignBtn').addEventListener('click',async()=>{if(!canEdit)return;const recurso_id=$('assignResource').value,servicioSel=$('assignService').value;if(!recurso_id)return msg('Elige quién atenderá los servicios.');msg(linkEditKey?'Guardando cambios…':'Guardando asignación…',true);if(servicioSel==='__all__' && !linkEditKey){const rows=services.map(s=>({negocio_id:negocioId,servicio_id:s.id,recurso_id}));const {error}=await sb.from('servicios_recursos').upsert(rows,{onConflict:'negocio_id,servicio_id,recurso_id',ignoreDuplicates:true});if(error)return msg('No pudimos asignar los servicios.');resetAssignForm();msg('Servicios asignados.',true);await load();flashReview();return}
  ...
  const {error}=await sb.from('servicios_recursos').upsert([{negocio_id:negocioId,servicio_id,recurso_id}],{onConflict:'negocio_id,servicio_id,recurso_id',ignoreDuplicates:true});if(error)return msg('No pudimos guardar la asignación.');resetAssignForm();msg(linkEditKey?'Asignación actualizada.':'Asignación guardada.',true);await load();flashReview()});
