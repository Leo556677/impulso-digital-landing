...
const LABEL_ALIASES = {
  ...
  ÁNGULO: 'angulo',
  TESIS: 'tesis',
  OBJETIVO_MENTAL: 'objetivo_mental',
  PUBLICO: 'publico',
  PÚBLICO: 'publico',
  ETAPA_FUNNEL: 'etapa_funnel',
  PRESENTADOR: 'presentador',
  PROFESIONAL_RESPONSABLE: 'profesional_responsable',
  ROL_DEL_PRESENTADOR: 'rol_presentador',
  FORMATO: 'formato',
  DURACION: 'duracion_seg',
  DURACIÓN: 'duracion_seg',
  DURACION_MASTER: 'duracion_seg',
  DURACIÓN_MASTER: 'duracion_seg',
  ...
  HOOK_SONORO: 'hook_sonoro',
  OPEN_LOOP: 'open_loop',
  REHOOKS: 'rehooks',
  RE_HOOKS: 'rehooks',
  PAYOFF: 'payoff',
  CTA_INTENCION: 'cta_intencion',
  CTA_INTENCIÓN: 'cta_intencion',
  CTA_FAMILY: 'cta_family',
  CTA: 'cta_master',
  CTA_MASTER: 'cta_master',
  DESTINO: 'cta_destino',
  ...
  STATUS: 'estado',
  ESTADO: 'estado',
  PARENT_CONTENT_ID: 'parent_content_code',
...
function normalizeRehooks(value) {
  if (Array.isArray(value)) {
    return value.map((item, index) => {
      if (typeof item === 'string') return { orden: index + 1, frase: clean(item) };
      return item;
    }).filter(Boolean);
  }
...
export function parseRegisterPackage(text) {
  ...
  for (const rawLine of lines) {
    ...
    if (!line.trim()) {
      if (activeKey && buffer.length) buffer.push('');
      continue;
    }
  ...
  if (!Object.keys(parsed).length) {
    throw new Error('No pude reconocer campos del REGISTER_PACKAGE. Usa el bloque entregado por el Content Engine.');
  }
  return parsed;
}
...
export function normalizeContentPayload(input, negocioId) {
  ...
  return {
    ...
    metadata: {
      source: clean(read(input, 'source')) || 'CONTENT_ENGINE',
...
export async function resolveContentContext({ redirect = true } = {}) {
  ...
  if (!session?.user) {
    if (redirect) location.replace('./cliente-acceso.html');
    throw new Error('Necesitas iniciar sesión.');
  }

  const { data: memberships, error: membershipError } = await sb
    .from('usuarios_negocio')
    .select('negocio_id,rol,created_at')
    .order('created_at');
  ...
  const requested = params.get('negocio');
  const saved = localStorage.getItem('impulso_negocio_activo');
  const pick = (id) => memberships.find((item) => String(item.negocio_id) === String(id));
  const membership = pick(requested) || pick(saved) || memberships[0];
  const negocioId = membership.negocio_id;

  localStorage.setItem('impulso_negocio_activo', String(negocioId));

  const { data: negocio, error: negocioError } = await sb
    .from('negocios')
  ...
  return {
    ...
    negocio,
    negocioId,
    canEdit: ['admin', 'propietario'].includes(membership.rol)
  };
}
...
export async function saveContent(input, context = null) {
  ...
  const parsed = typeof input === 'string' ? parseRegisterPackage(input) : input;
...
export async function updateContent(id, negocioId, patch) {
  ...
  delete safePatch.id;
  delete safePatch.negocio_id;
  delete safePatch.content_num;
  delete safePatch.content_code;
...
export async function retireContent(id, negocioId) {
  return updateContent(id, negocioId, { estado: 'RETIRED' });
}

export async function logoutContentVault() {
  await sb.auth.signOut();
  location.replace('./cliente-acceso.html');
}

// Puente estable para el futuro botón del generador web:
// import { saveContent, resolveContentContext } from './content-vault-client.js';
// await saveContent(registerPackageOrObject, context);
