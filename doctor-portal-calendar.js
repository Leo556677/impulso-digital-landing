(()=>{
  ...
  function card(item,list=false){
    const st=statusMeta(item),theme=themeFor(item.service,item.kind),stream=streamFor(item);
    ...
    if(list){
      return`<article class="cal-list-row ${esc(item.kind)} ${item.recorded?'is-recorded':''}" data-stream="${stream}" style="${cssVars(item)}" ${target} data-code="${esc(item.code)}" ${affordance}>
    ...
    return`<article class="cal-project ${esc(item.kind)} ${item.recorded?'is-recorded':''}" data-stream="${stream}" style="${cssVars(item)}" ${target} data-code="${esc(item.code)}" ${affordance} ${interactive?`aria-label="Abrir ${esc(item.title)}"`:''}>
