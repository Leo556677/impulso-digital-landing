...
(()=>{
  ...
  function renderServices(groups){
    ...
    host.innerHTML=list.length?list.map(g=>{const total=g.items.length,recorded=g.items.filter(x=>x.estado==='GRABADO').length,pending=total-recorded,pc=total?Math.round(recorded/total*100):0;return`<article class="card service-category jsService" data-service="${esc(g.key)}" tabindex="0" role="button" aria-label="Abrir ${esc(g.name)}"><div class="service-card-top"><div><div class="service-eyebrow">SERVICIO</div><h3>${esc(g.name)}</h3><p>${pending} ${pending===1?'proyecto pendiente':'proyectos pendientes'}</p></div><span class="service-arrow">${ic('right')}</span></div><div class="service-card-foot"><span>${recorded}/${total} grabados</span><span>${total} ${total===1?'guion':'guiones'}</span></div><div class="prog"><span style="width:${pc}%"></span></div></article>`;}).join(''):'<div class="card empty"><b>No tienes proyectos pendientes de grabación.</b><br>Cuando haya un guion listo aparecerá dentro de su servicio.</div>';
  ...
  renderRecord=async function(){
    ...
    if(h)h.textContent='Servicios para grabar';if(p)p.textContent='Entra a un servicio para ver sus proyectos y guiones pendientes.';
