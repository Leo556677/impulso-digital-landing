...
BEGIN
 ...
 IF jsonb_array_length(a->'parts')<>8 OR jsonb_array_length(a->'gates')<8 OR EXISTS(SELECT 1 FROM jsonb_array_elements(a->'parts') WHERE coalesce((value->>'score')::int,0)<80)
 ...
 metadata=(metadata-'recording_view_v1')||jsonb_build_object('script_history_v1',history,'editorial_audit_v1',a,'script_revision_v1',jsonb_build_object('schema_version','1.0','approved_script_version',v,'approved_checkpoint_id',p_checkpoint_id,'approved_at',t,'previous_approved_script_version',p_expected_script_version,'event_key',event_key,'approval_text',p_approval_text)),
