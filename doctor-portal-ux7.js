(()=>{
  ...
  renderVideos=function(items){
    videos.innerHTML=items.length?items.map((x,i)=>`<article class="card video jsV compact-video" data-i="${i}"><div class="vr"><span class="vn">${i+1}</span><span class="vo">DE ${items.length}</span><span class="pill ${x.estado==='GRABADO'?'ok':''}">${x.estado==='GRABADO'?ic('check'):ic('playi')} ${x.estado==='GRABADO'?'Grabado':'Pendiente'}</span></div><div class="video-row">${imgBox(firstImg(x),'Referencia de grabación',true)}<div class="vb"><h3>${esc(pt(x.pieza))}</h3><div class="meta"><span>${esc(x.pieza.servicio||'')}</span><span>${ic('clk')} ${x.pieza.duracion_seg?x.pieza.duracion_seg+' s':''}</span></div><button class="cta">${x.estado==='GRABADO'?'Revisar video':'Abrir y grabar'}</button></div></div></article>`).join(''):'<div class="card empty">Esta sesión todavía no tiene videos.</div>';
    wireImages(videos);document.querySelectorAll('.jsV').forEach(x=>x.onclick=()=>openVideo(+x.dataset.i));
  };
