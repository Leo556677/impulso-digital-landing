...
const services={S1:'Toxina botulínica',S2:'PRP facial',S3:'Limpieza facial',S4:'Liposucción de papada',S5:'Blefaroplastia',S6:'Rinoplastia'};
...
function matchTag(v){const c=v==='MATCH_FUERTE'?'strong':v==='MATCH_PARCIAL'?'partial':String(v).includes('HUECO')?'gap':'';
 const label=v==='MATCH_FUERTE'?'Encaja con el plan':v==='MATCH_PARCIAL'?'Encaja parcialmente':String(v||'').includes('HUECO')?'Tema por investigar':String(v||'').replaceAll('_',' ');
 return `<span class="tag ${c}">${esc(label)}</span>`}
function stateTag(v){const map={APPROVED:'Guion aprobado',RECORDED:'Grabado',READY:'Listo',PUBLISHED:'Publicado',PLANNED:'Planificado',NEEDS_CAPTURE:'Falta capturar material',NEEDS_SCRIPT:'Falta guion'};return `<span class="tag">${esc(map[v]||String(v||'').replaceAll('_',' '))}</span>`}
...
async function loadOrchestrator(){
 ...
 root().innerHTML=`
 <div class="notice"><b>Plan conectado.</b> El calendario define qué toca publicar; los recursos de guion ayudan a ejecutarlo sin cambiar la estrategia.</div>
 <section class="stats">${stat(m.episodes.length,'temas disponibles')}${stat(data.slots.length,'publicaciones planificadas')}${stat(`${strong}/${data.slots.length}`,'encajan con el plan')}${stat(gaps,'temas por investigar')}</section>
 <div class="grid"><section class="card"><h2>Próximas publicaciones</h2><p class="sub">${data.cal?`${fmtDate(data.cal.week_start)} – ${fmtDate(data.cal.week_end)} · ${data.cal.strategy_name}`:'No hay calendario activo.'}</p><div class="week">${data.slots.map(s=>slotHtml(s,m)).join('')||'<div class="empty">Sin slots.</div>'}</div></section>
 <aside><section class="card"><h2>Cómo fluye el trabajo</h2><p class="sub">Del negocio a la publicación, y de los resultados a la siguiente decisión.</p><div class="flow"><span>Datos del negocio</span><i>→</i><span>Temas</span><i>→</i><span>Calendario</span><i>→</i><span>Guion</span><i>→</i><span>Grabación</span><i>→</i><span>Publicación</span><i>→</i><span>Métricas</span></div></section>
 <section class="card" style="margin-top:16px"><h2>Estado del sistema</h2><div class="meta"><span class="pill ok">Sistema V2 activo</span><span class="pill">${lib.length} recursos disponibles</span><span class="pill">${rules.data?.length||0} reglas activas</span></div><p class="sub" style="margin-top:12px">${lib.filter(x=>x.metadata?.performance_status==='UNMEASURED').length} recursos todavía no tienen datos propios; se medirán cuando se usen en publicaciones.</p></section>
